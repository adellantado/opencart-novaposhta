      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <table class="form">
          <tr>
            ...
            <td><input type="text" name="novaposhta_api_key" value="<?php echo $novaposhta_api_key; ?>" /></td>
          </tr>
          <tr>
            <td><?php echo $entry_sender_organization; ?></td>
            <td><input type="text" name="novaposhta_sender_organization" value="<?php echo $novaposhta_sender_organization; ?>" /></td>
          </tr>
          <tr>
            <td><?php echo $entry_sender_person; ?></td>
            <td><input type="text" name="novaposhta_sender_person" value="<?php echo $novaposhta_sender_person; ?>" /></td>
          </tr>
          <tr>
...
<script type="text/javascript"><!--
      ...
      success: function (json) {
        ...
        for (i = 0; i < json.length; i++) {
          if (json[i]['city'] == '<?php echo $novaposhta_sender_city; ?>') {
            html += '<option selected="selected" value="' + json[i]['city'] + '">' + json[i]['city'] + '</option>';
          } else {
  ...
  function getWarehouses() {
    $.ajax({
      url: 'index.php?route=shipping/novaposhta/getWarehouses&token=<?php echo $token; ?>&filter=' + encodeURIComponent('<?php echo $novaposhta_sender_city; ?>'),
      dataType: 'json',
      success: function (json) {
